...
This is one of the integration POCs for the framework-agnostic preload API (Linear FD-537). The same pattern works in any React-rendering SSR framework (Astro, TanStack Start, Vike, Remix, etc.).
...
The default `.env.example` points at `https://example.fontdue.xyz`, which has CORS allow-listed `http://localhost:5173`. Point `VITE_FONTDUE_URL` at your own tenant if you have one — the client origin will need to be in your tenant's allowed origins.
